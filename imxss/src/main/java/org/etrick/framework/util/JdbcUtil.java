...
import org.etrick.framework.context.annotation.Column;
import org.etrick.framework.context.annotation.Table;
...
import org.etrick.framework.context.entity.SQLEntity;
...
public class JdbcUtil {
	...
	public static String parsPagerSQL(Pager pager) {
		// 封装分页条件
		if (StringUtil.isNullOrEmpty(pager.getCurrPage())) {
			pager.setCurrPage(1);
		}
		if (StringUtil.isNullOrEmpty(pager.getPageSize())) {
			pager.setPageSize(10);
		}
		Integer startRows = (pager.getCurrPage() - 1) * pager.getPageSize();
	 ...
	 * @return
	 */
	public static SQLEntity parseSQL(Object obj, Where where, Pager pager,
		...
		return new SQLEntity(sb.toString(), params.toArray());
	}
