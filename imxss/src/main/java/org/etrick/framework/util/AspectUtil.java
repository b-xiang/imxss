package org.etrick.framework.util;
...
public class AspectUtil {
	...
	public static void writeLog(String module) {
		...
		record.put(EtrickFinal.LOGGER_WRAPPER, module);
	...
	public static String getCurrLog() {
		Map<String, Object> record = getCurrRecord();
		String logHead = (String) record.get(EtrickFinal.LOGGER_WRAPPER);
		if (logHead == null) {
			return "";
		}
		return logHead;
	...
	public static String minusLog() {
		...
		if (tabs.length == 1) {
			record.put(EtrickFinal.LOGGER_WRAPPER, "");
			moduleThread.set(record);
			return "";
		}
		...
		record.put(EtrickFinal.LOGGER_WRAPPER, sb.toString());
		moduleThread.set(record);
		return sb.toString();
	}
	...
	public static void writeDBTemplate(String dbTemplate) {
		...
		record.put(EtrickFinal.JDBC_WRAPPER, dbTemplates);
		moduleThread.set(record);
	}
	...
	public static void minusDBTemplate() {
		...
		if (dbTemplates.size() <= 1) {
			record.put(EtrickFinal.JDBC_WRAPPER, null);
			moduleThread.set(record);
			return;
		}
	...
	public static void createDebugKey(String method) {
		Map<String, Object> record = getCurrRecord();
		record.put(EtrickFinal.SIMPLE_WRAPPER, method);
		moduleThread.set(record);
	}

	public static String getDebugKey() {
		Map<String, Object> record = getCurrRecord();
		Object method = record.get(EtrickFinal.SIMPLE_WRAPPER);
		return (String) method;
	}

	public static void cleanDebugKey() {
		Map<String, Object> record = getCurrRecord();
		record.remove(EtrickFinal.SIMPLE_WRAPPER);
		record.remove(EtrickFinal.LOG_WRAPPER);
		moduleThread.set(record);
	}
}
