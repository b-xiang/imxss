...
<title>用户注册 - ${setting.siteName }</title>
...
<body class="login-container">
	<div class="login-box">
		<div class="logo-img">
			<c:if test="${setting.openReg==1 }">
				<img src="${basePath }assets/img/logo.png" alt="" />
			</c:if>
		</div>
		<c:if test="${setting.openReg==0 }">
			<div class="am-form-group">
				<center>
					<label for="doc-vld-name-2">本站已关闭注册</label>
				</center>
			</div>
		</c:if>
		<c:if test="${setting.openReg==1 }">
			<form action="user/login.${defSuffix }" class="am-form"
				...
				<div class="am-form-group line">
					<label for="doc-vld-name-2"><i class="am-icon-comment"></i></label>
				...
				<c:if test="${setting.needInvite==1 }">
					<div class="am-form-group line">
						<label for="doc-vld-email-2"><i class="am-icon-hand-spock-o"></i></label> <input
							type="text" id="invite" name="invite" minlength="32"
							maxlength="64" placeholder="输入邀请码" required />
					</div>
				</c:if>
				<div class="am-form-group">
					<a class="toLogin" href="${basePath }login.${defSuffix}">已有账号,立即登录</a>
				</div>
				<button class="am-btn am-btn-secondary loginButton" type="submit">注册</button>
			</form>
		</c:if>
...
<script>
	function submitForm() {
		$.ajax({
			...
			url : '${basePath}user/reg.${defSuffix}',
			timeout : 60000,
